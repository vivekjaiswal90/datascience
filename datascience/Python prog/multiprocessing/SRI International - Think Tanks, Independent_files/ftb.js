...
function oiq_doTag() {
	...
	var req='http://px.owneriq.net/j/'+'?pt=ftb'+'&t='+encodeURI(t.join());
	if (window.oiq_pSource) req+='&s='+window.oiq_pSource;
	oiq_is(req);

	if (window.oiq_pUser) {
		var oiq_user_img = new Image();
		oiq_user_img.src = "http://px.owneriq.net/euid?pt=ftb&uid="+encodeURIComponent(window.oiq_pUser);
	}
	...
	if(oiq_parsedReferer) {
		if(oiq_parsedReferer.host && oiq_parsedReferer.protocol && oiq_parsedReferer.query) {
			if(oiq_parsedReferer.host.match(/google|bing|ask|aol/gi))	{
				...
				if(oiq_searchString) {
					var oiq_searchTagURL = 'http://px.owneriq.net/esq?pt=ftb&URL=';
